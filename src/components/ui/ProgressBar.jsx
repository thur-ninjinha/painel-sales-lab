    ...
    <div className="w-full">
      {(label || showPercent) && (
        <div className="flex justify-between items-center mb-1.5">
          {label && <span className="text-xs text-ink-muted truncate">{label}</span>}
          {showPercent && <span className="text-xs text-white font-bold ml-2 flex-shrink-0">{clamped.toFixed(0)}%</span>}
        </div>
      )}
